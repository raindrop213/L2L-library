  ...
  Promise.all([
    ...
    // 检查数组长度并添加占位符
    const maxLengthBeforeHR = Math.max(jaElementsBeforeHR.length, zhElementsBeforeHR.length);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('keydown', function(event) {
    ...
    debounceTimer = setTimeout(function() {
      const currentIndex = window.location.search ? parseInt(new URLSearchParams(window.location.search).get('line')) : 0;
      ...
      if (event.keyCode === 37 && currentIndex > 0) { // 左箭头键
        newIndex = currentIndex - 1;
      } else if (event.keyCode === 39 && currentIndex < maxIndex) { // 右箭头键
        newIndex = currentIndex + 1;
      }
